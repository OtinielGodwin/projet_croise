...
for (const country of countries) {
  const dir = `data_biomedical/${country}`;
  try { await Deno.mkdir(dir, { recursive: true }); } catch (e) {}

  for (let y = 1980; y <= 2026; y++) {
    for (const q of ["Q1", "Q2", "Q3", "Q4"]) {
      const url = `${BASE_URL}/archived/biomedical/${country}/${y}${q}`;
      try {
        const res = await fetch(url);
        if (res.status === 200) {
          const txt = await res.text();
          await Deno.writeTextFile(`${dir}/bio_${y}${q}.txt`, txt);
          console.log(`✅ BIO ${country}: ${y}${q}`);
        }
      } catch (e) {}
    }
  }
