...
for (const country of countries) {
  const dir = `data_sequences/${country}`;
  try { await Deno.mkdir(dir, { recursive: true }); } catch (e) {}

  for (let y = 1980; y <= 2026; y++) {
    for (const q of ["Q1", "Q2", "Q3", "Q4"]) {
      const url = `${BASE_URL}/archived/sequences/${country}/${y}${q}`;
      try {
        const res = await fetch(url);
        if (res.status === 200) {
          const content = await res.text();
          if (content.trim()) {
            await Deno.writeTextFile(`${dir}/seq_${y}${q}.fasta`, content);
            console.log(`✅ FASTA ${country}: ${y}${q}`);
          }
        }
      } catch (e) {}
    }
